...
class scDataset(object):
    def __init__(self, files):
        ...
        # self._datasets=[nc.Dataset(f, 'r') for f in files[1:]]

        # Set a few class variables
        d0 = nc.Dataset(files[0], 'r')
        ...
        timename = [
            dim for dim in d0.dimensions if d0.dimensions[dim].isunlimited()][0]
        ## Not sure if the above is more or less readable
        ...
        # Get the indices
        d0_time_len = d0.variables[timename].shape[0]
        fi = [0] * d0_time_len   # file (dataset) index
        li = [x for x in range(d0_time_len)]   # local time index
        varlists = {
            vname: [d0.variables[vname]]
            for vname in d0.variables
            if d0.variables[vname].dimensions[0] == timename
        }
        # for vname in d0.variables.keys():
        #     if d0.variables[vname].dimensions[0] == timename:
        #         varlists[vname] = [d0.variables[vname]]

        for di, f in enumerate(files[1:]):
            with nc.Dataset(f, 'r') as d:
                curlen = d.variables[timename].shape[0]
                fi.extend([di + 1 for x in range(curlen)])
                li.extend([x for x in range(curlen)])
                for vname in varlists:
                    varlists[vname].append(d0.variables[vname])

        # The first dimension must be the unlimited dimension, else we
        # pass through to the first dataset
        self.variables = OrderedDict([
            (vname, scVariable(varlists[vname], fi, li)
                if vname in varlists else d0.variables[vname])
            for vname in d0.variables
        ])
        # for vname in d0.variables.keys():
        #     if d0.variables[vname].dimensions[0] == timename:
        ...
        #         self.variables[vname] = scVariable(varlists[vname], fi, li)
        #     else:
